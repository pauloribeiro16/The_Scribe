...
def parse_nis2_from_text(file_path: str) -> List[Document]:
    """
    Parses the NIS2 Directive text file into granular, enriched, and self-contained Documents
    for each Recital and Article paragraph.
    """
    logging.info(f"Parsing NIS2 Directive text file with enrichment: {file_path}")
    if not os.path.exists(file_path):
        logging.error(f"NIS2 text file not found at: {file_path}")
        return []
    ...
    try:
        recitals_text, main_body = re.split(r'\s*HAVE ADOPTED THIS DIRECTIVE:\s*', full_text, 1)
    except ValueError:
        logging.error("Could not split NIS2 text at 'HAVE ADOPTED THIS DIRECTIVE:'.")
        return []
    ...
    for match in recital_matches:
        ...
        # Text Enrichment
        enriched_text = f"From the NIS2 Directive:\n### Recital {recital_num}\n\n{recital_text}"
        metadata = { "source_document": "NIS2", "type": "Recital", "recital_number": recital_num }
    ...
    for block in article_blocks:
        ...
        if para_matches:
            for i, match in enumerate(para_matches):
                ...
                metadata = {
                    "source_document": "NIS2", "type": "Article", "chapter": current_chapter,
    ...
    logging.info(f"Parsing of NIS2 complete. {len(llama_documents)} documents created.")
